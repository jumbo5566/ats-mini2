...
const uint16_t app_ver = 114;         //               EEPROM VER. If EEPROM read value mismatch (older), reset EEPROM    eeprom_ver_address    2
char app_date[] = "2025-04-13";
...
uint16_t currentApo = 0;             // Display sleep timeout, range = 0 to 255 in steps of 5
...
Band band[] = {
    {"VHF", FM_BAND_TYPE, 6400, 10800, 9050, 1, 0},
    {"MW1", MW_BAND_TYPE, 150, 1720, 810, 3, 4},
    {"MW2", MW_BAND_TYPE, 531, 1701, 621, 2, 4},
    ...
    {"40M", SW_BAND_TYPE, 6500, 7300, 7050, 0, 4},
    ...
    {"20M", SW_BAND_TYPE, 14000, 15000, 14270, 0, 4},
...
void drawMenu() {
  ...
  if (cmdMenu) {
    ...
    if (cmdApo) {
      spr.setTextColor(theme[themeIdx].menu_param,theme[themeIdx].menu_bg);
      spr.fillRoundRect(6+menu_offset_x,24+menu_offset_y+(2*16),66+menu_delta_x,16,2,theme[themeIdx].menu_bg);
      spr.drawNumber(currentApo,40+menu_offset_x+(menu_delta_x/2),60+menu_offset_y,4);
    }
  }
}
...
void drawSprite()
{
  ...
  if (cmdAbout) {
    ...
    spr.drawString("ESP32-SI4732 Receiver V2", 0, 0, 4);
    spr.setTextColor(theme[themeIdx].text, theme[themeIdx].bg);
    get_fw_ver();
    spr.drawString(fw_ver, 2, 33, 2);
    spr.drawString("https://github.com/jumbo5566/ats-mini2", 2, 33 + 16, 2);
...
void doApo( uint16_t v ) {
  if ( v == 1) {
    currentApo = currentApo + 1;
    if (currentApo > 255) currentApo = 255;
  } else {
    if (currentApo >= 5) currentApo = currentApo - 1;
    else currentApo = 0;
  }
  drawSprite();
}
...
void loop() {
  ...
  // Display sleep timeout
  if (currentApo) {
    if ((millis() - elapsedSleep) > currentApo * 1000 * 60) {
      OFF();
    }
  }
